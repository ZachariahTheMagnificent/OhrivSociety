...
namespace winsock
{
	...
	public:
		...
		/*!
		\brief
		Defines the various exceptions thrown by winsock.
		...
		*/
		...
		enum class Type
		{
			...
			An attempt was made to use an event object, but the specified handle is not
			...
			*/
			...
			An attempt was made to use a Windows Sockets function that directly maps to a Windows
			...
			*/
			...
			An attempt was made to use a Windows Sockets function which directly maps to a Windows
			...
			*/
			...
			An attempt was made to determine the status of an overlapped operation
			which is not yet completed. Attempts to use WSAGetOverlappedResult(with
			...
			*/
			...
			An attempt was made to initiate an overlapped operation that cannot be completed
			immediately. A completion indication will be given later when the operation has
			been completed. Note that this error is returned by the operating system, so
			...
			*/
			...
			broadcast permission being set using setsockopt(SO_BROADCAST). Another possible
			reason for the ACCES error is that when the bind function is called(on
			Windows NT 4.0 with SP4 and later), another application, service, or kernel
			mode driver is bound to the same address with exclusive access. Such exclusive
			access is a new feature of Windows NT 4.0 with SP4 and later, and is
			...
			*/
			...
			The system detected an invalid pointer address in an attempt to use a pointer
			argument of a call. This error occurs if an invalid
			pointer value was passed, or if the length of the buffer is too small. For instance, if the
			length of an argument, which is a sockaddr structure, is smaller than the
			sizeof ( sockaddr ).
			*/
			/******************************************************************************/
			FAULT,
			...
			*/
			...
			It is normal for WOULDBLOCK to be reported as the result from calling
			connect on a nonblocking SOCK_STREAM socket, since some time must elapse for
			...
			*/
			...
			socket) the function fails with the INPROGRESS error.
			*/
			/******************************************************************************/
			INPROGRESS,
			/******************************************************************************/
			...
			*/
			...
			from EAFNOSUPPORT. However, it is interchangeable in most cases, and all
			...
			*/
			...
			Typically, only one usage of each socket address (protocol/IP address/port) is permitted. This error occurs if an application attempts to bind a socket
			to an IP address/port that has already been used for an existing socket, or a
			socket that was not closed properly, or one that is still in the process of
			...
			usually need not call bind at all; connect chooses an unused port automatically.
			When bind is called with a wildcard address (involving ADDR_ANY), a
			ADDRINUSE error could be delayed until the specific address is committed.
			This could happen with a call to another function later, including connect,
			...
			*/
			...
			with NETRESET. Subsequent operations fail with CONNRESET.
			*/
			/******************************************************************************/
			CONNRESET,
			/******************************************************************************/
			...
			*/
			...
			A socket operation was attempted to an unreachable host. This usually means
			the local software knows no route to reach it.
			*/
			/******************************************************************************/
			HOSTUNREACH,
			/******************************************************************************/
			...
			*/
			...
			specified could not be found. This error is also returned if the service
			...
			*/
			...
			call to WaitForMultipleEvents fails or one of the registry functions fails
			...
			*/
			...
			BSD-compatible HOSTS, SERVICES, or PROTOCOLS files) could not be found, or
			a DNS request was returned by the server with a severe error.
			*/
			/******************************************************************************/
			NORECOVERY,
			/******************************************************************************/
			...
			is returned but neither an A record indicating the host itself exists nor is
			directly reachable.
			*/
			/******************************************************************************/
			NODATA,
			...
			*/
			...
			request was converted for local enforcement by the TC API. This could be due to
			an out of memory error or to an internal QoS provider error.
			*/
			/******************************************************************************/
			QOS_TRAFFIC_CTRL_ERROR,
			/******************************************************************************/
			/*!
			\brief
			QoS generic error.
			...
			*/
			...
			The value that the enum defaults to. Doesn't mean anything.
			*/
			/******************************************************************************/
			NILL
		};

		static Type ErrorCodeToEnum ( const int error_code ) noexcept;

		Exception ( ) = default;
		Exception ( const Type exception ) noexcept;
		Exception ( const int error_code ) noexcept;

		const char* what ( ) const noexcept override;
		Type GetType ( ) const noexcept;

	private:
		Type error_ { Type::NILL };
	};
}